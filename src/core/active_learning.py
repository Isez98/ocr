...
class ActiveLearningPipeline:
    ...
    def __init__(self, db_path: str = "ocr_learning.db", min_samples_for_retrain: int = 50):
    ...
    def log_correction(self, image_path: str, field_type: str, predicted_text: str, 
        ...
        try:
            ...
            print(f"Correction already exists for image {image_hash}")
    ...
    def prepare_training_data(self, output_dir: str = "training_data") -> Dict:
...
def integrate_with_ocr_service():
    """Example integration with the main OCR service"""
    
    learning_pipeline = ActiveLearningPipeline()
    
    # This would be added to your main process function in app.py
    def enhanced_ocr_with_learning(image_path: str, template_id: str, user_id: str = "api_user"):
        # ... existing OCR processing ...
        
        # After OCR processing, if confidence is low, flag for review
        results = []  # Your OCR results
        
        for field_name, result in results:
            confidence = result.get('confidence', 0.0)
            
            # Flag low confidence results for potential correction
            if confidence < 0.8:  # Threshold for review
                print(f"⚠️  Low confidence ({confidence:.2f}) for {field_name}: '{result['text']}'")
                print("Consider manual review and correction")
                
                # You could automatically save these for review
                # learning_pipeline.save_for_review(image_path, field_name, result['text'], confidence)
        
        return results
    
    # Correction endpoint (would be added to FastAPI)
    def submit_correction(image_path: str, field_type: str, predicted_text: str, 
                         correct_text: str, user_id: str):
        learning_pipeline.log_correction(
            image_path, field_type, predicted_text, correct_text, user_id=user_id
        )
...
if __name__ == "__main__":
    ...
    # Example correction
    # pipeline.log_correction(
    #     "samples/roi_patch.png", 
    #     "text", 
    #     "Chris", 
    #     "Chris Flores",
